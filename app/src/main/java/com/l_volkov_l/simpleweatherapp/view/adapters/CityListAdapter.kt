...
class CityListAdapter : BaseAdapter<GeoCodeModel>() {
    ...
    lateinit var clickListener: SearchItemClickListener //Паттерн функция обратного вызова(для нажатий на кнопку конкретного айтема)
       ...
       init {
        ...
        override fun bindView(position: Int) {
            mLocation.setOnClickListener {
                clickListener.showWeatherIn(mData[position])// при нажатии на город выводится его погода
            }
            ...
            mData[position].apply {
                mState.text = if(!state.isNullOrEmpty()){
                    itemView.context.getString(R.string.comma, state)
                }else ""
                mCity.text = name
